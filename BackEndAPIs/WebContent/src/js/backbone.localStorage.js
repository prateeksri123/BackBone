...
( function(root, factory) {
		...
		extend(Backbone.LocalStorage.prototype, {
			...
			findByNameAndPassword : function(model) {
				var result;
				var records = [];
				records = this.findAll();
				for (var i = 0, id, data; i < records.length; i++) {
					console.log(model.get("name") + "  " +  records[i].name + "  " +  model.get("pwd") + "  " +  records[i].pwd);
					if(model.get("name") == records[i].name && model.get("pwd") == records[i].pwd){
						result = records[i];
						break;
				}
				}
